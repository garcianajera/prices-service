...
├── domain
│   ├── model/        Price (record): brandId, productId, priceList, startDate, endDate,
│   │                 priority, amount (BigDecimal), currency
...
│   ├── port/         PriceRepositoryPort (outbound port): findCandidates(brandId, productId, applicationDate)
...
### 4.1 `Price`
- An immutable `record`. The compact constructor checks the invariants: fields are non-null,
  `endDate >= startDate`, `priority >= 0` and `amount >= 0`.
...
```
It returns the candidate prices. It does not order them or pick one.
...
- Every id is `BIGINT`/`Long`, to be consistent and to leave room to grow.
